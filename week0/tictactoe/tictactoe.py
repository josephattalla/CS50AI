...
def result(board, action):
    ...
    # if the action is placing a move on a spot that is not empty or not on the board, raise Excpetion
...
def minimax(board):
    ...
    if user == X:
        return max_function(board, -math.inf, math.inf)[0]
    else:
        return min_function(board, -math.inf, math.inf)[0]


def max_function(board, alpha, beta):
        """
        ...
        for move in moves:
            ...
            _, value = min_function(new_board, alpha, beta)
...
def min_function(board, alpha, beta):
    """
    ...
    for move in moves:
        ...
        _, value = max_function(new_board, alpha, beta)
